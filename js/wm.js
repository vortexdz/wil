...
base_mp3url = 'http://quran.ksu.edu.sa/ayat/mp3';
res_url = 'http://quran.ksu.edu.sa/ayat/resources';
res_ext = 'ayt';
masahef = {
...
Plugins.DBP.search = function(type , query , tash , cb , cbf){
	...
	var hp = url_interface + '&do=search&type='+type;
    if(typeof(tash) != undefined && ! tash){
        hp += "&remtash=1";
    }
	var xhr = $.post(hp , {query: query });
...
Plugins.DBP.amaken = function(mosshaf , page , cb){
	...
	$.get(url_interface+"&do=hilites&mosshaf="+mosshaf+"&page="+page, cb2, 'json');
}
...
SG.boot = function(cb){
	...
	SG.Trig.on("showToolsMenu" , function(){
		...
		$(window).on('hashchange.t' , function(){
