var Naver = (function (window, document, Math) {
...
/* REPLACE END: _move */
	...
	_end: function (e) {
		...
		if (this.directionLocked != 'v' && this._events.flick && duration < 300 && distanceX > 10 && distanceX < 200 && distanceY < 100 ) { // MOD Ayat (was the prev line)
		...
		if ( this.options.momentum && duration < 300 ) {
			if(this.options.momentumX){ // MOD Ayat (added this option)
			...
			if(this.options.momentumY){ // MOD Ayat (added this option)
			...
			time = Math.max((momentumX)?momentumX.duration:300, (momentumY)?momentumY.duration:300);
		...
		if ( this.options.snap ) {
			...
			if(this.options.snapX){ // MOD Ayat (added this option)
				newX = snap.x;
			}
			if(this.options.snapY){ // MOD Ayat (added this option)
				newY = snap.y;
			}
